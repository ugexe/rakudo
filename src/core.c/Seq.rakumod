...
my class Seq is Cool does Iterable does Sequence {
    ...
    multi method from-loop(&body, :$label, :$fire-last) {
        Seq.new: Rakudo::Iterator.Loop(&body, $label, $fire-last // 0)
    }
    multi method from-loop(&body, &cond, :$repeat!, :$label, :$fire-last) {
        Seq.new: $repeat
          ?? Rakudo::Iterator.RepeatLoop(&body, &cond // -> { 1 }, $label, $fire-last // 0)
          !! Rakudo::Iterator.WhileLoop(&body, &cond // -> { 1 }, $label, $fire-last // 0)
    }
    multi method from-loop(&body, &cond, :$label, :$fire-last) {
        Seq.new: Rakudo::Iterator.WhileLoop(&body, &cond // -> { 1 }, $label, $fire-last // 0)
    }
    multi method from-loop(&body, &cond, &afterwards, :$label, :$fire-last) {
        Seq.new: Rakudo::Iterator.CStyleLoop(&body, &cond // -> { 1 }, &afterwards, $label, $fire-last // 0)
    }
