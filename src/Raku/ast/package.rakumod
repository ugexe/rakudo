  ...
  is RakuAST::Package::Attachable
{
    ...
    # Two-call lifecycle: `Package.new` invokes this with no body (stub
    # path) before `to-parse-time` has bound `$!resolver`/`$!context`;
    # `package-def` invokes it again with the parsed body (finalize
    # path) after PERFORM-PARSE. Only the finalize path wraps the body
    # with the `(stubbed-meta-object, lexpad)` trailer, because
    # `stubbed-meta-object` memoizes and reaching for it before parser
    # state is bound would freeze in a wrong meta-object.
