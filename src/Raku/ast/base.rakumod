    ...
    method IMPL-FATALIZE-QAST($qast, $bool-context) {
        ...
        if nqp::istype($qast, QAST::Op) {
            ...
            if $op eq 'call' && nqp::istype($qast[0], QAST::WVal) && $qast[0].value =:= &FATALIZE {
                ...
                # whose fallback Failure a surrounding defined-or defuses). A
                # user-level transparent construct like the right of a defined-or
                # or a ternary branch is deliberately absent: fatal throws a
                # Failure there, as on the legacy frontend.
                self.IMPL-FATALIZE-QAST($_, $bool-context) for @($qast);
                 ...
                 # and FLATTENABLE_HASH calls): :flat must stay on the real
                 # result, not on a FATALIZE wrapper. The operand is still
                 # fatalized above.
                 if !$bool-context
            ...
            # A block marked by an explicit `no fatal` keeps its calls
            # unwrapped, along with everything nested under it.
            unless nqp::istype($qast, QAST::Block) && $qast.ann('no-fatalize') {
                self.IMPL-FATALIZE-QAST($_, 0) for @($qast);
            }
        }
    }
