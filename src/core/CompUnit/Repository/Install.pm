class CompUnit::Repository::Install does CompUnit::Repository::Locally does CompUnit::Repository::Installable {
...
sub MAIN(:$name is copy, :$auth, :$ver, *@, *%) {
    ...
    method install(Distribution::Interface $dist, :$force) {
        $!lock.protect( {
        my @*MODULES;
        ...
        for $dist.meta<provides>.kv -> $name, $relpath is copy {
            ...
            %provides{ $name } = {
                pm => {
                    :file($id),
                    :time(try $relpath.IO.modified.Num),
                    :$!cver
                }
            };
        ...
        my %files;
        for $dist.ls-files('bin') -> $relpath is copy {
            $relpath        = $is-win ?? ~$relpath.subst('\\', '/', :g) !! ~$relpath;
        ...
        for |$dist.meta<resources> -> $relpath is copy {
    ...
    method uninstall(Distribution::Interface $dist) {
